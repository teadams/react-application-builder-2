...
const Header = ({ title, className = "flex items-center justify-between p-5 rounded-t md:text-base sm:text-base text-base lg:text-lg font-family-besley text-[#18264C] font-semibold" }:
	{ title?: string, className?: string }) => {
	return (<div className={className}>{title}
	</div >)
}

const Modal = ({ children, key = "modal", title = "text",
	backgroundClassName
}: {
